...
const profileDetails = async function (req, res) {
  ...
  const updateProfile=async(req,res)=>{
    try{
        ...
        let checkeduserId = await userModel.findOne({ _id: userId })
       
        if (!checkeduserId) return res.status(404).send({ status: false, message: "User not Exist" })
    
        if(req["userId"]!=checkeduserId.userId){ return res.status(304).send({ status: false, message: "user not authorised to update profile" })}
...
module.exports = { createUser, loginUser,profileDetail,profileDetails,updateProfile }
